-- ============================================================================
-- Task 2 single-student review: EVENT -> SQL CATEGORY -> R BIN -> DOMINANT CATEGORY
--
-- HOW TO USE:
--   1. Set the student's id below (target_student CTE) -- this is the numeric
--      'id' value from the timeline CSVs, not a research_id.
--   2. Run this ENTIRE file. Only that one student's data comes back.
--   3. Export/inspect the result to review their session event-by-event.
--
-- OUTPUT: every activity event for the target student, with the SQL
-- category/timing, R bin boundaries/contributions, dominant bin category,
-- and marker flags -- exactly what feeds their row on the actual timeline.
...
target_student AS (
    SELECT 1 AS user_id  -- <<< CHANGE THIS: the student's numeric id to review
),

-- --------------------------------------------------------------------------
-- 1. Find every summarize_numbers.py file event used to define the task window.
-- --------------------------------------------------------------------------
task_file_events AS (
    SELECT
        r."user" AS user_id,
    ...
    WHERE (
            LOWER(REPLACE(COALESCE(f.old_file, ''), '\\', '/')) LIKE '%summarize_numbers.py%'
...
--    Normal Execution intervals are excluded from colour competition because
--    the updated R plot represents Execution as an exact-time marker instead.
--    If an Execution event was converted to Inactivity by the SQL rule, that
--    Inactivity interval remains eligible for the bin colour.
-- --------------------------------------------------------------------------
bin_category_totals AS (
    ...
    WHERE overlap_seconds > 0
      AND timeline_category <> 'Execution'
    GROUP BY
...
SELECT *
FROM validation
WHERE user_id = (SELECT user_id FROM target_student)
ORDER BY
    user_id,
    research_id,
    event_seq,
