-- ============================================================================
-- Task 3 single-student review: EVENT -> SQL CATEGORY -> R BIN -> DOMINANT CATEGORY
--
-- HOW TO USE:
...
-- --------------------------------------------------------------------------
-- 1. Find every classify_triangle.py file event used to define the task window.
-- --------------------------------------------------------------------------
task_file_events AS (
    SELECT
        r."user" AS user_id,
    ...
    WHERE (
            LOWER(REPLACE(COALESCE(f.old_file, ''), '\\', '/')) LIKE '%classify_triangle.py%'
